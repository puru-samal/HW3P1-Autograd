...
class RNNPhonemeClassifier(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, autograd_engine, num_layers=2):
    ...
    def forward(self, x, h_0=None):
        ...
        if h_0 is None:
            # NOTE: Because of the way gradient_buffer is set up, hidden must be a list of 
            # per-layer hidden states instead of a numpy array of shape (num_layers, batch_size, hidden_state)
            # Since, the same np.ndarray with diffrent views cannot be added to the gradient buffer,
            # this approach creates an independent np.ndarray for each hidded state which can
            # be succesfully added to the computational graph for gradient tracking
            hidden = [np.zeros((batch_size, self.hidden_size)) for _ in range(self.num_layers)]
        else:
            hidden = h_0
